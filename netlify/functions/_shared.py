"""Shared logic for all Netlify functions."""
import csv
import io
import re
import sys
...
def _load_keywords() -> dict:
    ...
    try:
        ...
        print(f"[KW] GSC fetch error: {e}", file=sys.stderr)
    try:
        uber = _fetch_csv(_UBER_URL)
    except Exception as e:
        print(f"[KW] Uber fetch error: {e}", file=sys.stderr)
...
def _plant_keywords_context(plant_name: str) -> str:
    ...
    for row in data.get("gsc", []):
        ...
        if not kw:
            continue
        matched = kw_matches(kw) or (plant_lower in plant_col or any(w in plant_col for w in plant_words))
        if not matched:
            continue
        try:
            ...
            gsc_hits.append({"kw": kw, "clicks": clicks, "impressions": impr,
                              "position": round(position, 1), "score": score})
        except Exception:
            pass
    ...
    for row in data.get("uber", []):
        ...
        try:
            ...
            uber_hits.append({"kw": kw, "volume": volume, "difficulty": diff, "score": score})
        except Exception:
            pass
...
def parse_all(text: str):
    ...
    print(f"[PARSE_FAILED] title={bool(title)} meta={bool(meta)} tab1={bool(tab1)} tab2={bool(tab2)}", file=sys.stderr)
...
[HEADER 5] - Bullet 5 text (gifting & DIY: connect THIS plant's specific visual trait or personality to a real gifting moment or DIY use)
...
- Bullet 5 MUST reference this specific plant's visual trait or personality and connect it to a concrete gifting moment or DIY project
...
def parse_amazon(text: str) -> dict:
    ...
    return {"product_name": product_name, "bullets": bullets[:5], "description": description, "keywords": keywords}
...
- Include: plant type (succulent/houseplant), size, use case (gift/indoor decor), and 1-2 style descriptors

DESCRIPTION rules:
- English only. Short sentences. One blank line between every section for mobile readability.
- Hook (2 sentences): emotional pull or problem-solution.
- What You'll Receive: bullet list with bullet symbol, covering plant name, size from base of pot, pot type and size
...
- Mix of: plant name variants, long-tail gifting phrases, aesthetic styles, and care type.
